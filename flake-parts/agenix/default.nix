...
}:
{
  ...
  config = {
    ...
    flake.nixosModules.security_agenix =
      ...
      with lib;
      let
        cfg = config.Sapphire.security.agenix;
      in
      {
        options.Sapphire.security.agenix = with types; {
    ...
    flake.homeModules.security_agenix =
      ...
      with lib;
      let
        cfg = config.Sapphire.hm.security.agenix;
      in
      {
        options.Sapphire.hm.security.agenix = with types; {
          enable = mkEnableOption ''
